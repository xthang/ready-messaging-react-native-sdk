...
import { logger as log, type LoggerType } from 'utils/logger'
...
export abstract class JobQueue<T> {
  ...
  constructor(options: Readonly<JobQueueOptions>) {
    ...
    assertDev(options.maxAttempts <= Number.MAX_SAFE_INTEGER, 'maxAttempts is too large')
    assertDev(options.queueType.trim().length, 'queueType should be a non-blank string')
   ...
   */
  async add(data: Readonly<T>, insert?: (job: ParsedJob<T>) => Promise<void>): Promise<Job<T>> {
    if (!this.started) {
  ...
  private async enqueueStoredJob(storedJob: Readonly<StoredJob>) {
    assertDev(storedJob.queueType === this.queueType, 'Received a mis-matched queue type')
    ...
    const { resolve, reject } = this.onCompleteCallbacks.get(storedJob.id) || {}
    ...
    const result: undefined | { success: true } | { success: false; err: unknown } = await queue.add(async () => {
      for (let attempt = 1; attempt <= this.maxAttempts; attempt += 1) {
        ...
        if (this.isShuttingDown) {
          log.warn(`${this.logPrefix} returning early for job ${storedJob.id}; shutting down`)
    ...
    assertDev(result, 'The job never ran. This indicates a developer error in the job queue')
    if (result.success) {
      resolve?.()
    } else {
      reject?.((result as any).err)
    }
  }

  async shutdown(): Promise<void> {
    const queues = this.getQueues()
    log.info(`${this.logPrefix} shutdown: stop accepting new work and drain ${queues.size} promise queues`)
