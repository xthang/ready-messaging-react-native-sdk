...
export class Sleeper {
   ...
   */
  sleep(ms: number, reason: string, options?: { resolveOnShutdown?: boolean }): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const shutdownCallback = () => {
        ...
        log.info(`Sleeper: resolving sleep task on shutdown. Original reason: ${reason}`)
        if (resolveOnShutdown) {
          setTimeout(resolve, 0)
        } else {
          setTimeout(() => {
            reject(new Error(`Sleeper: rejecting sleep task during shutdown. Original reason: ${reason}`))
          }, 0)
        }
      }
  ...
  shutdown(): void {
    ...
    log.info(`Sleeper: shutting down, settling ${this.shutdownCallbacks.size} in-progress sleep calls`)
    this.shuttingDown = true
    this.shutdownCallbacks.forEach((cb) => {
      try {
        cb()
      } catch (error) {
        log.error('Sleeper: Error executing shutdown callback', Errors.toLogFormat(error))
      }
    })
