...
export function concat<T>(...iterables: ReadonlyArray<Iterable<T>>): Iterable<T> {
...
export function filter<T, S extends T>(iterable: Iterable<T>, predicate: (value: T) => value is S): Iterable<S>
export function filter<T>(iterable: Iterable<T>, predicate: (value: T) => unknown): Iterable<T>
export function filter<T>(iterable: Iterable<T>, predicate: (value: T) => unknown): Iterable<T> {
  return new FilterIterable(iterable, predicate)
}

class FilterIterable<T> implements Iterable<T> {
  constructor(private readonly iterable: Iterable<T>, private readonly predicate: (value: T) => unknown) {}
...
class FilterIterator<T> implements Iterator<T> {
  constructor(private readonly iterator: Iterator<T>, private readonly predicate: (value: T) => unknown) {}
 ...
 */
export function collect<T, S>(iterable: Iterable<T>, fn: (value: T) => S | undefined): Iterable<S> {
  return new CollectIterable(iterable, fn)
}

export function collectFirst<T, S>(iterable: Iterable<T>, fn: (value: T) => S | undefined): S | undefined {
...
class CollectIterable<T, S> implements Iterable<S> {
  constructor(private readonly iterable: Iterable<T>, private readonly fn: (value: T) => S | undefined) {}
...
class CollectIterator<T, S> implements Iterator<S> {
  constructor(private readonly iterator: Iterator<T>, private readonly fn: (value: T) => S | undefined) {}
...
export function find<T>(iterable: Iterable<T>, predicate: (value: T) => unknown): undefined | T {
  for (const value of iterable) {
    if (predicate(value)) {
      return value
    }
...
export const isEmpty = (iterable: Iterable<unknown>): boolean => Boolean(iterable[Symbol.iterator]().next().done)
...
export function map<T, ResultT>(iterable: Iterable<T>, fn: (value: T) => ResultT): Iterable<ResultT> {
  return new MapIterable(iterable, fn)
}

class MapIterable<T, ResultT> implements Iterable<ResultT> {
  constructor(private readonly iterable: Iterable<T>, private readonly fn: (value: T) => ResultT) {}
...
class MapIterator<T, ResultT> implements Iterator<ResultT> {
  constructor(private readonly iterator: Iterator<T>, private readonly fn: (value: T) => ResultT) {}
...
export function* chunk<A>(iterable: Iterable<A>, chunkSize: number): Iterable<Array<A>> {
  let aChunk: Array<A> = []
...
class TakeIterable<T> implements Iterable<T> {
  constructor(private readonly iterable: Iterable<T>, private readonly amount: number) {}
...
export function zipObject<ValueT>(props: Iterable<string>, values: Iterable<ValueT>): Record<string, ValueT> {
